...
const CODIGO = 'PW769258045BR'; // Código padrão de rastreamento fornecido pelos correios
...
const getInfoPackage = async (body, codigo) => {
	...
	const status = Array.prototype.slice.call(dom.window.document.querySelectorAll('.listEvent')).map((row) => {
...
const start = async (codigo) => {
	const infoSavad = getPackageInfo(codigo);

	const body = await getBody(codigo);
	const info = await getInfoPackage(body, codigo);

	const lastStatus = getLast(info.status);
	const lastStatusSaved = infoSavad ? getLast(infoSavad.status) : undefined;

	if (!lastStatusSaved || !lastStatusSaved.descricao || lastStatus.descricao !== lastStatusSaved.descricao) await statusChanged(lastStatus, codigo);
